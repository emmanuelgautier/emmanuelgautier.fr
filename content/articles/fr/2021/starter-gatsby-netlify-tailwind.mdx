---
name: Starter Gatsby + Netlify CMS + TailwindCSS
description:
  Avec le nombre de sites web generés avec les mêmes outils, j'essaie au maximum
  de mutualiser l'effort de maintenance de la stack technique commune. Suivant
  ce principe, je viens de créer un starter qui vient du développement de mes
  autres sites.
image:
  contentUrl: /images/gatsby.png
keywords:
  - javascript
  - react
  - gatsby
  - tailwindcss
  - netlify
  - jamstack
identifier: starter-gatsby-netlify-tailwind
translationOfWork:
  "@id": gatsby-starter-netlify-tailwind
inLanguage: fr
isPartOf: blog
dateModified: 2022-03-20T11:51:52.061Z
dateCreated: 2021-10-17
---

Avec le nombre de sites web genérés avec les mêmes outils, j'essaie au maximum de mutualiser l'effort de maintenance de la stack technique commune. Suivant ce principe, je viens de créer un starter qui vient du développement de mes autres sites.

Vous pouvez retrouver le starter sur ce [Dépôt Github](https://github.com/emmanuelgautier/gatsby-starter-netlify-tailwind). Ce repo contient un exemple de site web fait avec :

- [Gatsby](https://www.gatsbyjs.org/)
- [Netlify CMS](https://www.netlifycms.org)
- [TailwindCSS](https://tailwindcss.com/)

Il suit les [principes JAMstack](https://jamstack.org) avec GIT comme source unique de vérité pour les pages et les articles de blog, déployé et délivré avec [Netlify](https://www.netlify.com).

Ce starter n'est pas complétement lié à Netlify, vous pouvez donc utiliser l'hébergeur que vous souhaitez. Cependant, comme ce starter utilise Netlify CMS, l'intégration est plus simple avec cet hébergeur.

## Quelles sont les principales fonctionnalitées ?

Ce starter apporte la liste de fonctionnalités suivante :

- Modèles de contenu pour pages et articles de blog
- Optimisé pour la performance avec des bons scores Web Vitals
- Optimisé pour le SEO
- Supporte les Twitter Cards et Open Graph
- Flux RSS
- Sitemap

Comme ce starter vient du développement de mes autres sites, cette liste devrait être suffisante pour créer un site basique. Pour autant, si vous pensez qu'une fonctionnalité est manquante, n'hésitez pas à contribuer en faisant une PR ou créer une issue avec votre proposition de nouvelle fonctionnalité.

Si vous êtes intéressé pour jeter un oeil, vous pouvez voir le rendu à ce lien, sur la **[Page Demo](https://gatsby-starter-netlify-tailwind.netlify.app/)**.
