...
image:
  contentUrl: /images/wamp.jpg
keywords:
  - php
  - windows
  - odbc
identifier: connecteurs-odbc-et-php
inLanguage: fr
isPartOf: blog
...
Comme vous le savez sans doute déjà, Windows met à disposition des développeurs un moyen simplifié de connexion aux bases de données. Ce moyen s'appel [ODBC](https://fr.wikipedia.org/wiki/Open_Database_Connectivity) pour Open DataBase Connectivity et est notament beaucoup utilisé par les outils BI. Cependant, il peut vous être utile de les utiliser au cours de votre développement web pour vous connecter à des bases de données sans avoir à rentrer certaines informations.
...
En PHP, deux solutions de connexion via ODBC existent. La première s'apparente plus à une connexion ressemblante à [`mysql_connect`](https://php.net/manual/fr/function.odbc-connect.php). Pour l'utiliser vous devez utiliser la fonction [`odbc_connect`](https://fr2.php.net/manual/en/function.odbc-connect.php) comme suivant :
...
L'autre solution est d'utiliser le classique objet [PDO](https://php.net/manual/fr/ref.pdo-odbc.php>). L'utilisation se fait comme suivant :
