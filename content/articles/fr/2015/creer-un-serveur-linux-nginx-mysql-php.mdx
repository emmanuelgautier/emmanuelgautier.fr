---
name: 'Creer un serveur Linux, Nginx, MySQL, PHP'
...
keywords:
  ...
  - sql
  - php
identifier: creer-un-serveur-linux-nginx-mysql-php
inLanguage: fr
isPartOf: blog
...
Attention les dépôts pris dans ce tutoriel fonctionnent pour la distribution Ubuntu. Pour toute autre distribution, vous pouvez trouver le lien vers les dépôts correspondants sur la [documentation](https://nginx.org/en/linux_packages.html)

```bash
sudo echo "deb https://nginx.org/packages/ubuntu/ $(lsb_release -sc) nginx
    deb-src https://nginx.org/packages/ubuntu/ $(lsb_release -sc) nginx" > /etc/apt/sources.list.d/nginx-stable.list
...
```bash
wget -O - https://nginx.org/keys/nginx_signing.key > key.gpg && sudo apt-key add key.gpg
```
