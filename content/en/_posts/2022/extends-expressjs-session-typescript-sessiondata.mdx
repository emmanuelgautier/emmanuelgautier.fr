...
Express Session provides a way to store data shared across differents HTTP Requests. The mechanism is to set a cookie with a session id, then retrieve and store data by this id during each request processing. This mechanism allows to store a lot of different data. There is a Typescript type named `SessionData` which allow to know what contains a session but not the data you will defined after without defining them.

To define what is the content of the SessionData type globally in the application, you have to define a file `global.d.ts` and declare the content of the typing. Here an example of can contains the `global.d.ts` defined:
...
This way to define session data is the same of every framework using express-session package. NestJS exposes a way to access session with express session or the fastify equivalent. This typing definition is exactly the same with NestJS.
